<?php
class Model_ss_admin extends Model_basemodel{
	//@override
	...
	*/
	public function update_login($userid,$user_pass=''){
		...
		if($user_pass){
			...
			$loginSucInfo['user_pass'] = md5($user_pass.SConstant::ADMINUSERKEY.$loginSucInfo['pass_random']);
		}
	 ...
	 * @param booler
	*/
    public function chage_pwd($userid,$newpwd){
        if(empty($userid) || empty($newpwd)){
            return false;
        }
        $where = array('user_id' => $userid);
        $info['pass_random'] = SUtil::random(6,0);
        $info['user_pass'] = md5($newpwd.SConstant::ADMINUSERKEY.$info['pass_random']);
