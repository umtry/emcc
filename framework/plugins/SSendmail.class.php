...
class SSendmail{
    ...
    */
	...
	public function useConfig($zone){
        ...
        $this->_sendServer = $host->host;
        $this->_port = $host->port;
        if(!empty($host->user)) {
            $this->_userName = base64_encode($host->user);
        }
        if(!empty($host->pass)) {
            $this->_password = base64_encode($host->pass);
        }
        $this->_from = $host->from;
        $this->_fromname = $host->fromname;
        return true;
    }
